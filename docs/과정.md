## 프로젝트 초기화
### 1. uv init generateclassdiagram
...
- `claude mcp add generateclassdiagram`: `generateclassdiagram` 이라는 이름으로 MCP 서버 등록
- `--scope project`
    - 서버 설정을 프로젝트 루트의 `.mcp.json` 파일로 생성
    - local(기본값)과 달리 파일로 남아서 버전 관리/공유가 됨(local은 `~/.claude.json`에 개인 설정으로만 저장, 공유 안 됨)
    - 특정 폴더 종속(Claude Code를 이 폴더에서 열 때만 서버가 잡힘)은 local과 공통 → user 스코프만 예외로 어디서 열든 항상 로드됨
...
- `GENERATECLASSDIAGRAM_ROOTS`엔 다이어그램을 뽑고 싶은 실제 프로젝트 경로 넣기
- `--directory`엔 위에서 clone한 이 서버 코드 위치
...
def _property_is_readonly(pd):
    ...
    for acc in acc_list.children:
        if acc.type != "accessor_declaration":
            continue
        # 접근자에 set / init 포함되어 있으면 False(읽기 전용 아님) 반환
...
def _declaration_lines(c: ParsedClass) -> list[str]:
    """class 선언. << >> 표시나 멤버가 있으면 { } 블록, 둘 다 없으면 한 줄"""
    name = _safe(c.name)
    mark = _mark(c)
